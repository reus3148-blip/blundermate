...
## 1. 라이브 분석 — 슬림 정보 바 + 하단 액션

1. 하단 nav "분석" 탭 → 빈 보드 라이브 분석 진입
2. 중간 바 (슬림 36px): `[수 평가 · 평가치(%)]` 가운데 정렬, 정보 전용
3. 하단 바 좌측: `[저장] [AI] [실행취소] [회전] [처음부터]` / 우측: `[<] [>]`. `[↩]` hidden
...
**왜:** Phase 62 — 중간 바 슬림화(52→36px) + nav를 하단 우측으로 복원. chess.com/lichess 컨벤션 정합 + 가장 자주 쓰는 prev/next를 썸 reach 영역으로. 라이브 6-btn + 메인복귀 동시 visible될 invariant 위반 (`syncBottomBar()` 단일 진입점이 차단).
