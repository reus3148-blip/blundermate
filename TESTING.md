...
## 1. 라이브 분석 — 하단 바 + 더보기 시트

1. 하단 nav "분석" 탭 → 빈 보드 라이브 분석 진입
2. 하단 바 좌측: `[저장] [AI] [⋯ 더보기]` 노출 / `[↩]` hidden / 우측: `[<] [>]` 노출
3. `⋯ 더보기` 탭 → 세로 시트 `[실행취소 · 회전 · 처음부터]` (한국어 라벨)
4. **회전** 선택 → 보드 흑 시점 (orientation-black)
5. **처음부터** 선택 → 시작 포지션 복귀 (32 pieces)
6. 보드 위 수 두기 → "분석 중..." placeholder + 잠시 후 엔진 라인 표시
7. **실행취소** → 마지막 수 제거 + 직전 위치로 복귀

**왜:** Phase 58에서 `liveActionBar` 통합 시 시트 dispatch 키 매칭 누락 가능. 더보기·메인복귀 동시 visible될 invariant 위반 (Phase 59에서 `syncBottomBar()` 단일 진입점화로 해소).
