...
## 1. 라이브 분석 — 하단 바 인라인 액션

1. 하단 nav "분석" 탭 → 빈 보드 라이브 분석 진입
2. 하단 바 좌측: `[저장] [AI] [실행취소] [회전] [처음부터]` 노출 / `[↩]` hidden / 우측: `[<] [>]` 노출
3. **회전** 탭 → 보드 흑 시점 (orientation-black)
4. **처음부터** 탭 → 시작 포지션 복귀 (32 pieces)
5. 보드 위 수 두기 → "분석 중..." placeholder + 잠시 후 엔진 라인 표시
6. **실행취소** 탭 → 마지막 수 제거 + 직전 위치로 복귀

**왜:** Phase 58/59의 시트 dispatch는 결국 인라인 노출로 회귀(Phase 60). 라이브 3-btn + 메인복귀 동시 visible될 invariant 위반 (`syncBottomBar()` 단일 진입점이 차단).
