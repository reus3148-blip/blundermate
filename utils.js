...
export function convertPvToSan(pv, fen) {
    if (!pv) return '';
    if (!safeLoad(pvChess, fen)) return '';
...
function getDefenders(fen, square) {
    ...
    if (testAtt) {
        if (!safeLoad(_defChess, flipStmInFen(fen, testAtt.color))) return [];
        ...
        if (!safeLoad(_defChess, flipStmInFen(fen, piece.color))) return [];
        try {
...
export function classifyMove(index, analysisQueue) {
    ...
    if (!move.engineLines || !move.engineLines[0]) {
        try {
            if (safeLoad(_classifyA, move.fen) && _classifyA.isCheckmate()) return 'Best';
        } catch {}
        return '';
    }
    ...
    if (classification === 'Best' && prevFen) {
        ...
        if (absoluteEvaluation >= 0 && !winningAnyways && !move.san?.includes('=')) {
            try {
                if (safeLoad(_classifyA, prevFen) && safeLoad(_classifyB, move.fen) && !_classifyA.isCheck()) {
                    ...
                    if (sacrificedPieces.length > 0) {
                        ...
                        outer: for (const sacked of sacrificedPieces) {
                            ...
                            for (const attacker of attackers) {
                                for (const promo of promotions) {
                                    if (!safeLoad(_captureChess, move.fen)) continue;
                                    let moved;
                                    try {
 ...
 */
// chess.js v1의 validateFen은 top-level 함수 — `{ ok, error? }` 반환.
// PGN 파싱 실패 시 fallback으로 FEN 입력을 수용하기 위한 것.
export function isValidFen(text) {
    const trimmed = (text || '').trim();
    if (!trimmed) return false;
    try {
        const res = validateFen(trimmed);
        return !!(res && res.ok);
    } catch { return false; }
}
