...
let _isGeminiEnabled = lsGet(GEMINI_KEY) !== 'false';
...
export function setIsGeminiEnabled(v) {
    _isGeminiEnabled = !!v;
    lsSet(GEMINI_KEY, _isGeminiEnabled);
}
...
export async function handleGeminiExplanation() {
    ...
    try {
        ...
        const geminiTextEl = document.getElementById('geminiText');
        const errHtml = `<p class="ai-error">${t('gemini_error')}<br><span class="ai-error-detail">${escapeHtml(error.message || '')}</span></p>`;
