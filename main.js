...
function startNewAnalysis(newQueue) {
    ...
    if (analysisQueue.length > 0) {
        persistentShapes = [];
        cg.set({ fen: 'start', drawable: { autoShapes: [] } });
...
function classifyMove(index) {
    ...
    // 해당 턴 플레이어 관점에서의 점수와 메이트 (> 0 이면 유리함)
    const prevPlayerMate = prevMate !== null ? (isWhite ? prevMate : -prevMate) : null;
    const currPlayerMate = currMate !== null ? (isWhite ? currMate : -currMate) : null;
    const prevPlayerScore = isWhite ? prevEval.scoreNum : -prevEval.scoreNum;
    const currPlayerScore = isWhite ? currEval.scoreNum : -currEval.scoreNum;
    ...
    if (prevLines && prevLines.length > 1 && prevLines[1]) {
        const bestMoveScore = isWhite ? prevLines[0].scoreNum : -prevLines[0].scoreNum;
        const secondBestScore = isWhite ? prevLines[1].scoreNum : -prevLines[1].scoreNum;
